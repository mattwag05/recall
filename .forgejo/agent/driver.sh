...
run_pi() {  # $1 = prompt (workdir = cwd) ; prints pi output to stdout
  ...
  jq -n --arg base "$LLM_BASE" --arg key "$LLM_KEY" '{
    providers: { local: {
      ...
      models: [ { id: "Qwen3.6-35B-A3B-4bit", name: "Qwen3.6-35B-A3B-4bit",
                  reasoning: false, input: ["text"],
                  cost: {input:0,output:0,cacheRead:0,cacheWrite:0},
                  contextWindow: 128000, maxTokens: 64000 },
                { id: "Qwen3.8-27B-4bit", name: "Qwen3.8-27B-4bit",
                  reasoning: false, input: ["text"],
                  cost: {input:0,output:0,cacheRead:0,cacheWrite:0},
                  contextWindow: 128000, maxTokens: 64000 } ]
    } } }' > "$pihome/.pi/agent/models.json"
  printf '{"defaultProvider":"local","defaultModel":"Qwen3.6-35B-A3B-4bit"}\n' > "$pihome/.pi/agent/settings.json"
